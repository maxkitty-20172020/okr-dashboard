...
async function main() {
  ...
  await prisma.$transaction(async tx => {
    const { quarter, week } = await ensurePeriodTree(tx);
    ...
    const objective = await tx.objective.create({ data: { title: "改善核心产品运营效率", description: "演示目标：降低广告成本并验证产品转化改进。", cycle: quarter.label, periodId: quarter.id, ownerId: li.id, keyResults: { create: { title: "ACOS 从 35% 降至 25%", baselineValue: 35, targetValue: 25, currentValue: 30, direction: "DECREASE", unit: "%" } } }, include: { keyResults: true } });
    const taskInclude = { owner: true, collaborators: true } as const;
    const business = await tx.task.create({ data: { title: "验证核心产品主图方案", area: "BUSINESS", kind: "LONG_TERM", ownerId: li.id, scope: "美国站 · 厨房线", completionCriteria: "取得测试结果，确定采用哪版主图并记录依据。", status: "WAITING", latestProgress: "两版主图已就绪，测试运行中。", nextAction: "按约定日期收集结果", nextFollowUpAt: future(7), dueAt: future(14), lastConfirmedAt: new Date(), keyResultId: objective.keyResults[0].id, schedulePeriodId: week.id, milestones: { create: [{ title: "准备两版主图", done: true }, { title: "取得测试结果", dueAt: future(12) }, { title: "确定最终方案", dueAt: future(14) }] } }, include: taskInclude });
    const people = await tx.task.create({ data: { title: "新人独立接手产品线", area: "PEOPLE", kind: "LONG_TERM", ownerId: wang.id, completionCriteria: "通过独立操作验收，并完成任务交接。", status: "IN_PROGRESS", latestProgress: "已完成基础培训，准备独立操作验收。", nextAction: "与业务负责人确认验收标准", nextFollowUpAt: future(2), dueAt: future(10), lastConfirmedAt: new Date(), schedulePeriodId: week.id, collaborators: { create: { userId: li.id, deliverable: "提供业务验收标准", dueAt: future(2) } } }, include: taskInclude });
    const inventory = await tx.task.create({ data: { title: "确认旺季备货与出货排期", area: "INVENTORY", kind: "LONG_TERM", ownerId: zhao.id, priority: 2, scope: "英国站 · 收纳线", completionCriteria: "确认销量预测、补货数量及供应商出货排期。", status: "BLOCKED", health: "AT_RISK", latestProgress: "销售预测已交付，供应商尚未确认档期。", riskNote: "可能影响备货节点，需要主管协调采购确认备用方案。", nextAction: "与采购确认供应商档期", nextFollowUpAt: future(0), dueAt: future(20), lastConfirmedAt: new Date(), schedulePeriodId: week.id, collaborators: { create: [{ userId: li.id, deliverable: "提供销售预测", done: true, note: "预测清单已交付。" }, { userId: wang.id, deliverable: "协调旺季人员与备岗", dueAt: future(5) }] } }, include: taskInclude });
    for (const task of [business, people, inventory]) {
      await tx.taskUpdate.create({ data: { taskId: task.id, authorId: task.ownerId, kind: "CREATED", body: "初始化演示事项。" + task.latestProgress, snapshot: snapshot(task) } });
      await ensureTaskRaciDefaults(tx, { id: task.id, ownerId: task.ownerId, collaborators: task.collaborators ?? [] });
    }
    await backfillDeliverablesFromCollaborators(tx);
